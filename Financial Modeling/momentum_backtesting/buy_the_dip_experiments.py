...
SECTOR_UNDERPERF_DAYS = 30          # rolling window for stock vs sector comparison
...
STOP_LOSS_PCT         = 0.10       # Exit B: 20% below entry price
...
CHART_OUTPUT_PATH     = 'momentum_backtesting/buy_the_dip_charts_experiments.png'
...
def score_signal(sig):
    ...
    if sig['conviction_score']:
        score += 1

    # prefer deeper short-term dips
    if sig['ret_1m'] is not None:
        score += min(max(-sig['ret_1m'] * 10, 0), 1)

    if sig['ret_2m'] is not None:
        score += min(max(-sig['ret_2m'] * 5, 0), 1)
...
# SIGNAL EVALUATION
# ──────────────────────────────────────────────────────────────────────────────
def _rolling_return_drop(ticker, prices, sim_date, window_days):
    """
    Returns % change over last window (negative = drop).
    Example: -0.08 = stock is down 8%
    """
    ...
    if len(col) < window_days + 5:
        return None

    start_price = float(col.iloc[-window_days])
    end_price = float(col.iloc[-1])
...
def evaluate_entry(ticker, prices, sim_date):
    ...
    # ── Condition A ───────────────────────────────────────────────────────────
    ret_1m = _rolling_return_drop(ticker, prices, sim_date, 21)
    ret_2m = _rolling_return_drop(ticker, prices, sim_date, 42)

    result['ret_1m'] = ret_1m
    result['ret_2m'] = ret_2m

    # Condition A: must be down 5–10% in either window
    cond_1 = ret_1m is not None and -0.10 <= ret_1m <= -0.05
    cond_2 = ret_2m is not None and -0.10 <= ret_2m <= -0.05

    result['cond_A'] = cond_1 or cond_2
...
def run_backtest(prices, start_date, end_date, label=''):
    ...
    for sim_date in trading_days:
        ...
        if n_open < MAX_POSITIONS and cash > 0:
            ...
            candidates.sort(key=lambda s: -score_signal(s))
            ...
            for sig in candidates[:slots_available]:
                ...
                # Compute 52-week high as of entry date for exit-A threshold
                _col = prices[tkr].dropna()
                _col = _col[_col.index <= ts]
                high_52 = float(_col.iloc[-252:].max()) if len(_col) >= 252 else (float(_col.max()) if not _col.empty else p)

                positions[tkr] = {
                    'entry_price':          p,
                    'shares':               shares,
                    'entry_date':           sim_date,
                    'entry_1m_ret':         sig['ret_1m'],
                    'entry_2m_ret':         sig['ret_2m'],
                    'high_52wk_at_entry':   high_52,
                }
                entry_events.append((sim_date, tkr, p))
                last_open_date = sim_date

                if VERBOSE:
                    ret1m_str = f"{sig['ret_1m']:.1%}" if sig['ret_1m'] is not None else 'N/A'
                    ret2m_str = f"{sig['ret_2m']:.1%}" if sig['ret_2m'] is not None else 'N/A'
                    print(f"  {sim_date}  [ENTRY]  {tkr:6s}  "
                          f"price={p:.2f}  1M={ret1m_str}  2M={ret2m_str}")
                    print(f"    Cond A (dip 5–10%):       {'YES' if sig['cond_A'] else 'NO '}  "
                          f"1M={ret1m_str} 2M={ret2m_str}")
